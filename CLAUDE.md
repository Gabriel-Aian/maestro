...
14. **Bug de verdade achado na primeira execução real do `.github/workflows/release.yml`, não hipotético**: o job terminou verde (testes, typecheck, build e "publish" todos com sucesso) e mesmo assim o app relatou `404` ao checar atualizações (`GET .../releases.atom`). Causa: `electron-builder` publica como **rascunho (draft)** por padrão — `releaseType` não estava setado em `electron-builder.yml`. Um draft não aparece nem em `GET /releases/latest` nem no feed `releases.atom` que `electron-updater` consulta, então do ponto de vista do app não existe Release nenhuma. Efeito colateral pior: como um draft não cria a tag Git de verdade até ser publicado, ao subir o segundo artefato (depois do `.exe`, o `latest.yml`) o `electron-builder` não achou a Release recém-criada pela tag e criou OUTRA — confirmado ao vivo: duas Releases `v0.3.1`, ambas draft, do mesmo job. Corrigido com `releaseType: release` em `electron-builder.yml` (publica de vez, sem intervenção manual, exatamente o que o workflow automático promete). As Releases draft órfãs desse primeiro run tiveram que ser apagadas manualmente pelo usuário no GitHub (nenhuma ferramenta disponível aqui cria/apaga Release, só lista/lê) — se voltar a acontecer alguma duplicata, é sinal de que `releaseType: release` foi removido ou de uma corrida parecida, não repita o padrão de "criar/achar release por asset" sem essa flag.
...
electron-builder.yml        # instalador Windows — asar:false é deliberado, ver "Regras que o código assume"
```
...
**`electron-builder.yml` empacota SEM asar (`asar: false`) — não "arrume" isso sem entender por quê.** `dist/cli.js` (e o `node_modules` de que ele depende: `commander`, `node-cron`, `pino`, `playwright-core`, `zod` — só produção, `electron`/`vite`/etc. ficam de fora) precisa continuar sendo arquivos de verdade no disco depois de instalado, porque quem os executa é o Agendador de Tarefas do Windows via `schtasks`/`cmd.exe` — processos comuns do SO, sem a menor ideia do que é um `.asar`. Ativar asar de novo quebraria os agendamentos silenciosamente (o resto do app funcionaria normalmente, já que `out/main/index.js` já embute `src/` inteiro via bundling do electron-vite — só a invocação do `schedule tick` depende de `dist/` existir como pasta real).
...
**Canal de auto-update é GitHub Releases PÚBLICO — decisão explícita do usuário, não default técnico.** `electron-builder.yml`'s `publish: { provider: github, owner: Gabriel-Aian, repo: maestro }` funciona sem token embutido no app só porque as Releases são públicas; um canal privado exigiria embutir um token de leitura dentro do próprio `.exe` distribuído, legível por qualquer um que o abra — não faça essa troca sem confirmar de novo com o usuário. **Isso não elimina o trabalho de lançar uma versão**: alguém (ou o CI) ainda precisa gerar o instalador e publicá-lo a cada atualização; o que o auto-update remove é só o usuário final ter que baixar e reinstalar manualmente. Nunca testado publicando uma Release de verdade neste ambiente (exigiria `GH_TOKEN` real e a intenção explícita de publicar algo público) — o que foi confirmado é a checagem/download/instalação do lado do app (`autoUpdater`, IPC, UI) contra o `app.isPackaged` corretamente detectado como falso em desenvolvimento.

**Publicar uma Release é automático a cada merge em `main` — `.github/workflows/release.yml`, não mais um passo manual.** Decisão do usuário, explicitamente para não ter que rodar `npm run package:win:publish` na mão a cada atualização: todo push em `main` (o que uma "Merge pull request" no GitHub produz) dispara um job em runner `windows-latest` de verdade (sem contorno de wine — isso só existe no ambiente de dev Linux) que builda e publica sozinho, usando o `GITHUB_TOKEN` automático do próprio Actions (nunca um token pessoal — é gerado e destruído a cada execução, nunca passa pela máquina de ninguém; `permissions: contents: write` no workflow é o que autoriza esse token a criar a Release). `npm run package:win` (local, sem publicar) e `npm run package:win:publish` (manual, exige `GH_TOKEN` próprio) continuam existindo para builds fora desse fluxo — ver "Comandos". **`electron-builder.yml`'s `publish.releaseType: release` é obrigatório** — sem essa linha o electron-builder publica como rascunho por padrão, invisível para `electron-updater` e para `GET /releases/latest` (bug 14 em "Estado atual", achado ao vivo na primeira execução real deste workflow).
**Versionamento dessa publicação automática é calculado, nunca commitado de volta.** O workflow lê `major.minor` do `package.json` do próprio repositório e sobrescreve só o `patch` com `github.run_number` (`npm pkg set version=...`, editado apenas no checkout efêmero do runner — nunca dá `git commit`/`git push` de volta para `main`). `run_number` só cresce a cada execução deste workflow, então a versão publicada é sempre estritamente maior que a anterior, que é exatamente o que `electron-updater` precisa para saber que existe algo mais novo. Consequência: o `version` commitado em `package.json` (hoje `0.3.0`) só muda quando alguém bump manualmente `major`/`minor` para marcar um marco de verdade — o `patch` ali nunca reflete o que foi publicado, é só ponto de partida para o cálculo. Não "conserte" isso fazendo o workflow commitar a versão de volta: geraria um push em `main` a cada release, dependeria de `main` não estar protegida contra push direto, e ainda arriscaria um loop (push do bot disparando o workflow de novo).
...
4. **Telas da casca Electron, na ordem combinada com o usuário: ~~fila/histórico~~ ~~perfis/navegadores~~ ~~fluxos~~ ~~pesquisas~~ ~~agendamentos~~ ~~configurações~~ — todas prontas.** Padrão usado em todas: `electron/shared/ipc.ts` (contrato) → handler em `electron/main/*Ipc.ts` (busca do núcleo real, sem caminho paralelo) → método no preload → tela em `electron/renderer/src/screens/`.
...
5. **~~Empacotamento: electron-builder + instalador Windows.~~ Feito** (`electron-builder.yml`, `npm run package:win`) — testado de ponta a ponta neste ambiente via `wine` (ver "Estado atual" e "Comandos"). O que ainda falta, não bloqueante para gerar um instalador funcional:
   ...
   - **~~Auto-update.~~ Implementado** (`electron-updater`, ver "Regras que o código assume" e o bug 13 em "Estado atual") — canal GitHub Releases público, decisão explícita do usuário, publicação automática a cada merge em `main` via `.github/workflows/release.yml`. O que ainda falta:
     - **Nunca publicada uma Release de verdade.** O workflow nunca rodou de fato ainda (só existe a partir do merge deste PR) — confirme na aba Actions do GitHub, após o primeiro merge em `main`, que o job `release` termina verde e que a Release aparece com o `.exe` e o `latest.yml` anexados.
