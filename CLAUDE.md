...
Fases 1 a 4, 6 e 7 do plano estão implementadas. 87 testes passando, typecheck limpo.
...
| Casca Electron — fundação + telas de fila, histórico, perfis/navegadores, fluxos, pesquisas e agendamentos | Tela de configurações |
...
└── renderer/src/
    ...
    ├── screens/           # uma tela por arquivo (QueueScreen, HistoryScreen, ProfilesScreen, FlowsScreen, SearchesScreen, SchedulesScreen, ...)
...
- `test/orchestrator.test.ts` — atomicidade por perfil de `enqueueSearches` (nenhum job é enfileirado se qualquer perfil do lote for inválido, mesmo com outros válidos); e que um agendamento registra `lastStatus: 'failed'` mesmo quando o job lança antes de produzir um `RunResult`.
